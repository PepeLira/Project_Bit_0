...
#define KEY_STATUS_CTRL_BIT	BIT(0)
#define KEY_STATUS_SHIFT_BIT	BIT(1)
#define KEY_STATUS_ALT_BIT	BIT(2)
#define KEY_STATUS_FN_BIT	BIT(3)
...
#define INT_STATUS_CTRL_CHANGE		BIT(1)
#define INT_STATUS_SHIFT_CHANGE		BIT(2)
#define INT_STATUS_ALT_CHANGE		BIT(3)
#define INT_STATUS_FN_CHANGE		BIT(4)
#define INT_STATUS_KEY_EVENT		BIT(5)
#define INT_STATUS_MOUSE_EVENT		BIT(6)
#define INT_STATUS_POWER_BTN		BIT(7)
...
struct lyra_kbd_data {
	...
	/* Track if we synthesized SHIFT for each keycode */
	bool shift_synthesized[MAX_KEYCODES];
	
	/* Track active mouse direction keys for movement (keycodes 49-52) */
	bool mouse_up_active;
	bool mouse_down_active;
	bool mouse_left_active;
	bool mouse_right_active;

	/* Track active scroll keys when SHIFT is held with 49-52 */
	bool scroll_up_active;
	bool scroll_down_active;
	bool scroll_left_active;
	bool scroll_right_active;
...
/* Track which FN keys need SHIFT synthesized for shifted characters */
static const bool fn_needs_shift[MAX_KEYCODES] = {
	[7] = true,   /* underscore (_) */
	[9] = true,   /* plus (+) */
	[14] = true,  /* double quote (") */
	[15] = true,  /* left brace ({) */
	[17] = true,  /* right brace (}) */
	[24] = true,  /* colon (:) */
	[26] = true,  /* question mark (?) */
	[36] = true,  /* tilde (~) */
	[38] = true,  /* pipe (|) */
};
...
static const unsigned short keymap_normal[MAX_KEYCODES] = {
	...
	KEY_RESERVED,	/* 49: FN9 - MOUSE_DOWN (movement, not a key) */
	KEY_RESERVED,	/* 50: FN10 - MOUSE_UP (movement, not a key) */
	KEY_RESERVED,	/* 51: FN11 - MOUSE_RIGHT (movement, not a key) */
	KEY_RESERVED,	/* 52: FN12 - MOUSE_LEFT (movement, not a key) */
};
...
static const unsigned short keymap_shift[MAX_KEYCODES] = {
	...
	KEY_RESERVED,	/* 49: FN9 - MOUSE_SCROLL_DOWN (scroll, not a key) */
	KEY_RESERVED,	/* 50: FN10 - MOUSE_SCROLL_UP (scroll, not a key) */
	KEY_RESERVED,	/* 51: FN11 - MOUSE_SCROLL_RIGHT (scroll, not a key) */
	KEY_RESERVED,	/* 52: FN12 - MOUSE_SCROLL_LEFT (scroll, not a key) */
};
...
static const unsigned short keymap_fn[MAX_KEYCODES] = {
	...
	KEY_DOWN,	/* 49: FN9+FN = DOWN arrow */
	KEY_UP,		/* 50: FN10+FN = UP arrow */
	KEY_RIGHT,	/* 51: FN11+FN = RIGHT arrow */
	KEY_LEFT,	/* 52: FN12+FN = LEFT arrow */
};
					...
					bool pressed)
{
	...
	/* Debug logging */
	dev_dbg(&kbd->client->dev, "Key event: code=%d pressed=%d shift=%d alt=%d fn=%d\n",
		 keycode, pressed, shift, alt, fn);
	
	/*
	 * Modifier keys (FN:37, RSHIFT:41, LSHIFT:25, ALT:33, CTRL:30) are
	 * tracked by firmware in REG_KEY_STATUS and handled by
	 * lyra_kbd_sync_modifiers(). Ignore their FIFO events to prevent
	 * conflicts between FIFO and register state.
	 */
	if (keycode == 25 || keycode == 41 || keycode == 30 || 
	    keycode == 33 || keycode == 37) {
		return;
	}
	
	/*
	 * Handle mouse movement/scroll keys (49-52).
	 * In normal mode: mouse movement
	 * In shift mode: mouse scroll
	 * In FN mode: arrow keys (handled normally below)
	 */
	if (keycode >= 49 && keycode <= 52) {
		if (!fn) {
			/* Normal/Shift: Mouse movement or scroll */
			switch (keycode) {
			case 49: /* DOWN */
				if (shift) {
					/* Scroll down */
					kbd->scroll_down_active = pressed;
					if (pressed)
						input_report_rel(kbd->mouse_input, REL_WHEEL, -1);
				} else {
					/* Mouse movement down */
					kbd->mouse_down_active = pressed;
				}
				break;
			case 50: /* UP */
				if (shift) {
					/* Scroll up */
					kbd->scroll_up_active = pressed;
					if (pressed)
						input_report_rel(kbd->mouse_input, REL_WHEEL, 1);
				} else {
					/* Mouse movement up */
					kbd->mouse_up_active = pressed;
				}
				break;
			case 51: /* RIGHT */
				if (shift) {
					/* Scroll right (horizontal scroll) */
					kbd->scroll_right_active = pressed;
					if (pressed)
						input_report_rel(kbd->mouse_input, REL_HWHEEL, 1);
				} else {
					/* Mouse movement right */
					kbd->mouse_right_active = pressed;
				}
				break;
			case 52: /* LEFT */
				if (shift) {
					/* Scroll left (horizontal scroll) */
					kbd->scroll_left_active = pressed;
					if (pressed)
						input_report_rel(kbd->mouse_input, REL_HWHEEL, -1);
				} else {
					/* Mouse movement left */
					kbd->mouse_left_active = pressed;
				}
				break;
			}
		...
		/* FN mode: Fall through to normal key handling (arrow keys) */
	}
	
	/*
	 * Determine if we need to synthesize SHIFT for FN layer.
	 * Some FN layer characters (like +, {, }, ?, ~, |) require SHIFT
	 * but we want them without user pressing SHIFT.
	 */
	if (fn && fn_needs_shift[keycode] && !shift) {
		synthesize_shift = true;
	}
	...
	if (pressed) {
		...
		/* Synthesize SHIFT press if needed for FN characters */
		if (synthesize_shift) {
			input_report_key(kbd->kbd_input, KEY_LEFTSHIFT, true);
			kbd->shift_synthesized[keycode] = true;
		}
		...
		/* Release synthesized SHIFT if we added it on press */
		if (kbd->shift_synthesized[keycode]) {
			input_report_key(kbd->kbd_input, KEY_LEFTSHIFT, false);
			kbd->shift_synthesized[keycode] = false;
		}
...
static void lyra_kbd_process_fifo(struct lyra_kbd_data *kbd)
{
	...
	dev_dbg(&kbd->client->dev, "Processing FIFO cycle start\n");
	...
	while (i < FIFO_MAX_READ) {
		...
		dev_dbg(&kbd->client->dev, "FIFO[%d]: raw=0x%02x type=%d code=%d\n",
		...
		case FIFO_EVENT_HOLD:
			 ...
			 * GPIO keys (FN1-FN8).
			 */
			dev_dbg(&kbd->client->dev, "Ignoring HOLD event for keycode %d\n", keycode);
			goto next;
		default:
...
next:
	...
	if (i > 0)
		dev_dbg(&kbd->client->dev, "FIFO processing done. Processed %d events.\n", i);
}
...
static void lyra_kbd_process_mouse(struct lyra_kbd_data *kbd)
{
	...
	/*
	 * Read mouse deltas from hardware.
	 * Firmware provides normalized deltas:
	 *   ±2 for straight movement (single direction)
	 *   ±1 for diagonal movement (two directions)
	 *   0 when no movement keys pressed
	 */
	...
	/* Apply speed multiplier to hardware deltas */
	if (delta_x != 0) {
	...
	/* Emit repeated scroll events while shift+direction keys are held */
	if (kbd->scroll_up_active)
		input_report_rel(kbd->mouse_input, REL_WHEEL, 1);
	if (kbd->scroll_down_active)
		input_report_rel(kbd->mouse_input, REL_WHEEL, -1);
	if (kbd->scroll_right_active)
		input_report_rel(kbd->mouse_input, REL_HWHEEL, 1);
	if (kbd->scroll_left_active)
		input_report_rel(kbd->mouse_input, REL_HWHEEL, -1);

	if (kbd->scroll_up_active || kbd->scroll_down_active ||
	    kbd->scroll_left_active || kbd->scroll_right_active)
		input_sync(kbd->mouse_input);
}
...
static void lyra_kbd_sync_modifiers(struct lyra_kbd_data *kbd)
{
	...
	key_status = (u8)ret;
	ctrl = (key_status & KEY_STATUS_CTRL_BIT) != 0;
	shift = (key_status & KEY_STATUS_SHIFT_BIT) != 0;
	alt = (key_status & KEY_STATUS_ALT_BIT) != 0;
	fn = (key_status & KEY_STATUS_FN_BIT) != 0;

	/* 
	 * Report all modifier states based on register, not FIFO events.
	 * CTRL, SHIFT, and ALT map to their respective LEFT variants.
	 * FN is tracked but not reported to input subsystem (non-standard key).
	 */
	input_report_key(kbd->kbd_input, KEY_LEFTCTRL, ctrl);
	input_report_key(kbd->kbd_input, KEY_LEFTSHIFT, shift);
	input_report_key(kbd->kbd_input, KEY_LEFTALT, alt);
	input_sync(kbd->kbd_input);
	
	dev_dbg(&kbd->client->dev, "Synced modifiers: ctrl=%d shift=%d alt=%d fn=%d\n",
		ctrl, shift, alt, fn);
}
...
static void lyra_kbd_poll_work(struct work_struct *work)
{
	...
	if (int_status & (INT_STATUS_CTRL_CHANGE | INT_STATUS_SHIFT_CHANGE | 
			  INT_STATUS_ALT_CHANGE | INT_STATUS_FN_CHANGE))
	...
	/* Process mouse events or synthesized movement from direction keys */
	if ((int_status & INT_STATUS_MOUSE_EVENT) || 
	    kbd->mouse_up_active || kbd->mouse_down_active ||
	    kbd->mouse_left_active || kbd->mouse_right_active ||
	    kbd->scroll_up_active || kbd->scroll_down_active ||
	    kbd->scroll_left_active || kbd->scroll_right_active)
...
static int lyra_kbd_setup_input_devices(struct lyra_kbd_data *kbd)
{
	...
	__set_bit(REL_HWHEEL, mouse_input->relbit);
